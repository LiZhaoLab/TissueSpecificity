...
#from: Neme and Tautz 2014: http://gentree.ioz.ac.cn/download/dm6_ver78_age.tsv
...
p1<-ggplot(unique(flydata2[,c("gene", "Age", "tau", "medtau")]), aes(x=Age, y=tau)) +geom_violin(aes(fill=factor(Age)))+theme_classic()+
 scale_fill_manual(values=c("#b87e39",
"#6287da",
"#67ad61",
"#8653aa",
"#b94971"))+ geom_boxplot(aes(group=factor(Age)), outlier.shape=NA,width=0.1)+ylab("Tau")+
  xlab(sprintf('Gene age group (Young \u2192 Old)'))+ theme(legend.position="none")+theme(axis.text=element_text(size=10))+geom_text(aes(y = medtau,x=Age, label = sprintf("%0.2f",round(medtau,2))),position=position_dodge(width = 0.8),size = 2.5,vjust=1.5, check_overlap = T)

rm(flydata2)
#let's now look at the proportion of genes with max expression in testis and ovary


proptestisovary<- flydata %>% group_by(Age,  maxtissue) %>% summarise(n=n()/25) %>%mutate(proportion=n/sum(n), total=sum(n)) 
...
p2<-ggplot(subset(proptestisovary, Tissue %in% c("Testis", "Ovary", "Male Carcass", "Female Carcass")), aes(x=Age, y=proportion, group=Tissue, col=Tissue, fill=Tissue))+theme_classic()+
  ...
  geom_errorbar(aes(ymin=min, ymax=max), width=0.05)+theme(axis.text=element_text(size=10), legend.text=element_text(size=10))+xlab("Gene age group")+scale_color_manual(values=c("#b87e39",
"#6287da",
"#67ad61",
"#8653aa",
"#b94971"))
...
p3<-ggplot(subset(propexpressed, Tissue %in% c("Female Carcass", "Male Carcass", "Testis", "Ovary")), aes(x=Age, y=prop, group=Tissue, col=Tissue, fill=Tissue))+theme_classic()+
  geom_point(aes(fill=Tissue))+geom_line()+ylab("Proportion of \n genes expressed with \n FPKM>2 in tissue")+
  geom_errorbar(aes(ymin=min, ymax=max), width=0.05)+theme(axis.text=element_text(size=10), legend.text=element_text(size=10))+xlab("Gene age group")+scale_color_manual(values=c("#b87e39",
"#6287da",
"#67ad61",
"#8653aa",
...
p1<-ggplot(flydata, aes(x=Age, y=logFPKM))+geom_boxplot(outlier.shape=NA,  width=0.5, aes(fill=Age))+
  theme_classic()+facet_wrap(~Tissue)+ylab("Log2(FPKM+1)")+
   ylim(0,21)+
  theme(axis.text=element_text(size=10))+
  #scale_y_continuous(breaks=c(0,.25,0.5,.75,1),limits = c(0,1.5))+
  xlab("Gene age group")+stat_pvalue_manual(stat.test)+theme(axis.text.x=element_blank())+scale_fill_manual(values=c("#b87e39",
"#6287da",
"#67ad61",
"#8653aa",
"#b94971"))
...
p2<-ggplot(res, aes(x = reorder(Tissue, -statistic), y=statistic))+geom_bar(stat="identity",fill =ifelse(res$Tissue=="Testis", "#b94971", ifelse(res$Tissue=="Ovary", "#6287da","grey")))+theme_classic() + theme(legend.position='none', axis.text.x=element_text(angle=90), axis.title.x=element_blank())+scale_fill_hue(c=45, l=40)+ylab("ANOVA F statistic") + xlab("Tissue")


p3<-ggplot(res, aes(x = reorder(Tissue, -sum), y=sum))+geom_bar(stat="identity",fill =ifelse(res$Tissue=="Testis", "#b94971", ifelse(res$Tissue=="Ovary", "#6287da","grey")))+theme_classic() + theme(legend.position='none', axis.text.x=element_text(angle=90), axis.title.x=element_blank())+scale_fill_hue(c=45, l=40)+ylab("Summed difference\nof group means") 
...
p1<-ggplot(tmp4,aes(x= Age, y=connectivity ))+geom_boxplot(aes( fill=Tissue), outlier.alpha=0.1)+
       stat_pvalue_manual(data=stat.test1, label="p.adj= {format.pval(p.adj)}",y.position=c(26,30,22), x="Age")+theme_classic()+theme(axis.text=element_text(size=10))+
 xlab("Gene age group")+ylab("Relative transcription factor activity")+geom_text(aes(y = med,x=Age, group=Tissue, label = sprintf("%0.2f",round(med,2))),position=position_dodge(width = 0.8),size = 3, vjust = -0.4, check_overlap = T)+scale_fill_manual(values=c("#b87e39",
"#6287da",
"#67ad61",
"#8653aa",
"#b94971"))
...
p2<-ggplot(tmp4, aes(x=connectivity, y=logFPKM, color=Tissue)   )+geom_point(alpha=0.05)+stat_smooth(aes(fill=Tissue, linetype=Tissue), method="loess",color="black", alpha=0.8)+
 # xlim(0,20)+
  theme_classic()+xlab("Activity of a genes TF \n partners in tissue")+ylab("Gene expression in tissue (Log2 FPKM)")+scale_fill_manual(values=c("#b87e39",
"#6287da",
"#67ad61",
"#8653aa",
"#b94971"))+scale_color_manual(values=c("#b87e39",
"#6287da",
"#67ad61",
"#8653aa",
"#b94971"))
...
p5<-ggplot(DroTF, aes(x=Age, y=nInteractions))+geom_violin(aes(fill=Age))+geom_boxplot(fill='white',outlier.alpha=0, width=0.1)+ 
  ...
  theme_classic()+theme(legend.position='none',axis.text=element_text(size=10))+xlab("Gene age")+ylab("Number of TFs")+geom_text(aes(y = medtf,x=Age, label = medtf),position=position_dodge(width = 0.8),size = 3, vjust = -0.6, check_overlap = T)+scale_fill_manual(values=c("#b87e39",
"#6287da",
"#67ad61",
"#8653aa",
"#b94971"))+ylim(0,65)
...
#p7
#ggsave(plot = p7, "~/Box Sync/Evan/Zhao lab notebook/Writing/Tissue specificity paper/200820_Figure3.pdf", width=10, height=8)
```




```{r Figure 4, fig.width=8, fig.height=10}
...
p1<-ggplot(propTissue, aes(x=Age, y=prop, group=Tissue, col=Tissue, fill=Tissue))+theme_classic()+theme(axis.text=element_text(size=10), legend.text=element_text(size=10))+
  geom_point(aes(fill=Tissue))+geom_line()+ylab("Proportions of genes with ATAC peaks \n in promoters")+
  geom_errorbar(aes(ymin=min, ymax=max), width=0.1)+ylim(0,1)+scale_color_manual(values=c("#b87e39",
"#6287da",
"#67ad61",
"#8653aa",
"#b94971"))
...
p2<-ggplot(subset(tissuepeaks, Tissue=="Testis"), aes(x= isPeak, y=FPKM,fill=isPeak))+geom_violin()+geom_boxplot(width=0.2, fill="white", outlier.shape = NA)+stat_compare_means(label= "p.signif", label.x.npc=0.2)+ylab("Testis FPKM")+theme_classic()+labs(fill="Testis ATAC peak")+theme(axis.text=element_text(size=10),legend.position='none', axis.title=element_text(size=10))+xlab("Does gene have ATAC-seq \n peak in testis?")+ylim(0,50)+geom_text(aes(y = med,x=isPeak, label = sprintf("%0.2f",round(med,2))),position=position_dodge(width = 0.8),size = 2.5,vjust=-0.8, check_overlap = T)+scale_fill_manual(values=c("#b87e39",
"#6287da",
"#67ad61",
"#8653aa",
"#b94971"))

p3<-ggplot(subset(tissuepeaks, Tissue=="Ovary"), aes(x= isPeak, y=FPKM, fill=isPeak))+geom_violin()+geom_boxplot(width=0.2, fill="white", outlier.shape=NA)+stat_compare_means(label= "p.signif", label.x.npc=0.2)+ylab("Ovary FPKM")+theme_classic()+labs(fill="Ovary ATAC peak")+theme(axis.text=element_text(size=10),legend.position='none', axis.title=element_text(size=10))+xlab("Does gene have ATAC-seq \n peak in ovary?")+ylim(0,50)+geom_text(aes(y = med,x=isPeak, label = sprintf("%0.2f",round(med,2))),position=position_dodge(width = 0.8),size = 2.5,vjust=1.5, check_overlap = T)+scale_fill_manual(values=c("#b87e39",
"#6287da",
"#67ad61",
"#8653aa",
"#b94971"))
...
p4<-ggplot(subset(tissuepeaks, Tissue=="Male Carcass"), aes(x= isPeak, y=FPKM, fill=isPeak))+geom_violin()+geom_boxplot(width=0.2, fill="white", outlier.shape=NA)+stat_compare_means(label= "p.signif", label.x.npc=0.2)+ylab("Male Carcass FPKM")+theme_classic()+labs(fill="S2 ATAC peak")+theme(axis.text=element_text(size=10), legend.position='none',axis.title=element_text(size=10))+xlab("Does gene have ATAC-seq \n peak in S2 cells?")+ylim(0,50)+geom_text(aes(y = med,x=isPeak, label = sprintf("%0.2f",round(med,2))),position=position_dodge(width = 0.8),size = 2.5,vjust=-1, check_overlap = T)+scale_fill_manual(values=c("#b87e39",
"#6287da",
"#67ad61",
"#8653aa",
"#b94971"))
...
p7<-ggplot(subset(tmp4, Tissue=="Testis"), aes(x=isPeak, y=connectivity, fill=isPeak ))+geom_violin()+geom_boxplot(width=0.15, fill="white", outlier.shape=NA)+theme_classic()+stat_compare_means(method = "wilcox", label = "p.signif", label.x.npc = 0.5)+ylab("TF activity in testis") + xlab("Does gene have promoter ATAC-seq \npeak in testis?")+theme(legend.position='none')+geom_text(aes(y = med,x=isPeak, label = sprintf("%0.2f",round(med,2))),position=position_dodge(width = 0.8),size = 2.5,vjust=-1.5, check_overlap = T)+scale_fill_manual(values=c("#b87e39",
"#6287da",
"#67ad61",
"#8653aa",
"#b94971"))

p8<-ggplot(subset(tmp4,  Tissue=="Ovary"), aes(x=isPeak, y=connectivity, fill=isPeak ))+geom_violin()+geom_boxplot(width=0.15, fill="white", outlier.shape=NA)+
  theme_classic()+stat_compare_means(method="wilcox", label = "p.signif", label.x.npc = 0.5)+ylab("TF activity in ovary") + xlab("Does gene have promoter ATAC-seq \npeak in ovary?")+theme(legend.position='none')+geom_text(aes(y = med,x=isPeak, label = sprintf("%0.2f",round(med,2))),position=position_dodge(width = 0.8),size = 2.5,vjust=-1.5, check_overlap = T)+scale_fill_manual(values=c("#b87e39",
"#6287da",
"#67ad61",
"#8653aa",
"#b94971"))
...
ggplot(tmp4, aes(x=isPeak, y=FPKM,fill=connectivitybin))+geom_boxplot(outlier.shape=NA)+stat_compare_means(aes(group=connectivitybin),label="p.signif")+facet_wrap(~Tissue)+theme_classic()+labs(fill="TF activity", y="FPKM", x="Does gene have detectable open chromatin in its promoter?")+ylim(0,80)+geom_text(aes(y = med,x=isPeak, group=connectivitybin, label = round(med,2)),position=position_dodge(width = 0.8),size = 3, vjust = -0.6,check_overlap = T)+scale_fill_manual(values=c("#b87e39",
"#6287da",
"#67ad61",
"#8653aa",
"#b94971"))
